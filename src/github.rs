...
fn fetch_prs(args: &[&str]) -> Result<Vec<PrItem>> {
    ...
    let items: Vec<PrItem> =
        serde_json::from_str(&json).context("Failed to parse gh JSON output")?;
    Ok(items)
}
...
fn sync_topic(
    ...
    for pr in prs {
        ...
        let url = format!(
            ...
            pr.repository.name_with_owner, pr.number
        );
    ...
    println!(
        "  {} — {} added, {} updated, {} merged/closed",
        topic_name, added, updated, closed
    );
    Ok(())
}
...
fn is_pr_open(url: &str) -> bool {
    ...
    let Ok(pr) = serde_json::from_str::<PrState>(&json) else {
        return true;
    };
