...
pub fn start(kind: SyncKind, db_path: String, model_dir: PathBuf) -> mpsc::Receiver<SyncMsg> {
    ...
    std::thread::spawn(move || {
        match run(kind, &db_path, &model_dir, &tx) {
            Ok(())  => { let _ = tx.send(SyncMsg::Done); }
            Err(e)  => { let _ = tx.send(SyncMsg::Error(e.to_string())); }
        }
    });
    rx
}

fn run(kind: SyncKind, db_path: &str, model_dir: &PathBuf, tx: &mpsc::Sender<SyncMsg>) -> Result<()> {
    let db = Database::open(db_path)?;
    let embedder = if model_dir.exists() { Embedder::load(model_dir).ok() } else { None };
    let report = |msg: &str| { let _ = tx.send(SyncMsg::Status(msg.to_string())); };
